...
Meilisearch v1.13 introduces Vector Store Stabilization, Dumpless Upgrade, Improves the Indexing Speed by granularly selecting the filter features and Proxy Search Requests.

TBD explain better and with more marketing sentences
...
After being released in v1.3.0, we have now fully stabilized the Vector Store feature. The experimental feature no longer needs to be enabled, and the `vectorStore` field is no longer shown or accepted by the `/experimental-features` route.
