...
⚠️ The experimental feature API can break between Meilisearch versions. Be aware of this if using it in production.
...
Previously, the documents with attributes containing the query words at the beginning of the attribute would be considered more relevant than documents containing the query words at the end of the attributes.

With this change the ranking rule considers the distance to the position of the word in the query rather than the absolute distance to the beginning of the document.
...
* Enhance Separator-based Tokenization (#215) @ManyTheFish
  ...
  - brackets `{([])}` are no more considered as context separators so words separated by brackets are now considered near together for the proximity ranking rule
...
* Expose lastUpdate and isIndex in `/stats/` endpoint (#3851) @gentcys
...
- [Meilisearch](https://github.com/meilisearch/meilisearch): @cuishuang, @jirutka, @vvv, @0xflotus, @gentcys
