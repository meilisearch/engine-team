...
This experimental feature can be enabled at runtime using our new `/experimental-features` endpoint.
```bash
❯ curl \
...
### Similarity score
As part of v1.3.0 we will return a similarity score as part of the documents retrieved. This score will be between 0 and 1 and viewed under the `_semanticSimilarity` property. The higher the ranking score, the more similar the document.
...
🗣️ Since this feature is experimental, we need your help to iterate on it! You are more than welcome to share your thoughts and your feedback on [this discussion](https://github.com/meilisearch/meilisearch/discussions/621).
...
This experimental feature can be turned on at runtime using our new `/experimental-features` endpoint.
```bash
❯ curl \
  -X PATCH 'http://localhost:7700/experimental-features/' \
  -H 'Content-Type: application/json'  \
...
🗣️ Since this feature is experimental, we need your help to iterate on it! You are more than welcome to share your thoughts and your feedback on [this discussion](https://github.com/meilisearch/meilisearch/discussions/594).
...
* Remove the `docid_word_positions` database. Reduces the index size. (#3819) @loiclec
