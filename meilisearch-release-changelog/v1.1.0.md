...
🧰 Most integrations (SDKs, clients, libraries) will be compatible with this version between 4 to 48 hours after the release.
...
`facetStats` only lists facets containing numerical values. Facets with other values are not included in the `facetStats` object. This object contains the minimum (`min`) and the maximum (`max`) of each numerical facet:
...
Done in #3347 and on [Charabia](https://github.com/meilisearch/charabia/releases/tag/v0.7.1) by @choznerol, @cymruu, @james-2001 and @ManyTheFish.
