...
This is not your average boring release! This brand new release introduces a tons of exciting features including the demanded multi-index search! You can now do one single HTTP request to perform different searches in multiple indexes.<br>
Also, already existing features have been pimped: the geo-search with the introduction of the `geoBoundingBox`, the addition of the facet stats, the CSV import including the sperator customization... Read the full changelog to be sure not to miss anything!
...
Meilisearch can now perform searches in multiple indexes in a single HTTP request
...
In this case, Meilisearch returns an array of results for each index:
...
* GitHub CIs & test suite
  ...
  * Make clippy happy for Rust 1.67, allow `uninlined_format_args` (#3434) @dureuill -> TBD
* Documentation
